<template>
    <div class="body">
        <el-form :model="mdl" ref="ruleForm" label-width="130px">
            ...
            <el-form-item :label="$i18n.t('BANK_NAME')" required :error="errors.name">
                <el-input v-model="mdl.name" :placeholder="$i18n.t('PLEASE_INPUT', { value: $i18n.t('BANK_NAME') })"></el-input>
            </el-form-item>
            <el-form-item :label="$i18n.t('BANK_CODE')" required :error="errors.code">
                <el-input v-model="mdl.code" :placeholder="$i18n.t('PLEASE_INPUT', { value: $i18n.t('BANK_CODE') })"></el-input>
            </el-form-item>
...
<script>
    import { addBankCode, editBankCode } from '@/api/countryBankCode'
        ...
        methods: {
            submitForm () {
                ...
                if (this.mdl.id === 0) {
                    ...
                    editBankCode(this.mdl.id, this.mdl).then(() => {
                        // 关闭抽屉
                        this.handleCloseDrawer()
                        this.$Message.success(this.$i18n.t('HANDLE_SUCCESS'))
                        // 调用爷爷组件刷新数据
