<template>
    <div class="body">
        <el-form :model="mdl" ref="ruleForm" label-width="130px">
            <el-form-item :label="$i18n.t('LANGUAGE')" required :error="errors.locale">
                <el-select v-model="mdl.locale" :value="mdl.locale">
            ...
            <el-form-item :label="$i18n.t('BANNER_IMAGE')" required :error="errors.image">
                <el-upload
                    action=""
...
<script>
    import { addInvitation, editInvitation } from '@/api/invitation'
        ...
        methods: {
            ...
            submitForm () {
                ...
                if (this.mdl.id === 0) {
                    ...
                    editInvitation(this.mdl.id, this.mdl).then(() => {
                        // 关闭抽屉
                        this.handleCloseDrawer()
                        this.$Message.success(this.$i18n.t('HANDLE_SUCCESS'))
                        // 调用爷爷组件刷新数据
