<template>
    <div class="body">
        <el-form :model="mdl" ref="ruleForm" label-width="130px">
            ...
            <el-form-item :label="$i18n.t('LEVEL_NAME')">
                <span>{{ mdl.user_level.name_text }}</span>
            </el-form-item>
            <el-form-item :label="$i18n.t('PAY_AMOUNT')">
                <span>{{ mdl.amount }}</span>
            </el-form-item>
            <el-form-item :label="$i18n.t('RECHARGE_TRADE_NO')">
                <span>{{ mdl.trade_no }}</span>
            </el-form-item>
            <el-form-item :label="$i18n.t('USER_MANUAL_RECHARGE_IMAGE')">
...
<script>
    import { auditManualRecharge } from '@/api/userManalRecharge'
    ...
    export default {
        ...
        methods: {
            submitForm () {
                ...
                this.errorMessage = ''
                auditManualRecharge(this.mdl).then(() => {
                    // 关闭抽屉
