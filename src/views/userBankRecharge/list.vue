...
<!--                <el-button  type="primary" icon="el-icon-plus" @click="handleDownload">{{ $i18n.t('CREATE_USER_LEVEL') }}</el-button>-->
            ...
            <el-table border style="width: 100%;" stripe :data="data" v-loading="loading">
                ...
                <el-table-column align="center" prop="amount" :label="$i18n.t('RECHARGE_AMOUNT')" :min-width="100">
                    <template slot-scope="scope">
                ...
                <el-table-column align="center" prop="admin_id" :label="$i18n.t('ADMIN_ID')" :min-width="150" ></el-table-column>
                <el-table-column align="center" prop="created_at" :label="$i18n.t('CREATE_TIME')" :min-width="150"></el-table-column>
                <el-table-column align="center" prop="updated_at" :label="$i18n.t('USER_TASK_AUDIT_TIME')" :min-width="150">
                    <template slot-scope="scope">
...
<script>
    ...
    import { bankRechargeCsv } from '@/api/csv'
        ...
        computed: {
            formItems () {
                ...
                return [
                    ...
                    { title: this.$i18n.t('RECHARGE_TIME'), type: 'date-range', key: 'created_at' },
                    { title: this.$i18n.t('USER_TASK_AUDIT_TIME'), type: 'date-range', key: 'updated_at' }
                ]
            },
            isShowDivider () {
        ...
        methods: {
            ...
            formatJson (filterVal, jsonData) {
                return jsonData.map(v => filterVal.map(j => {
                    if (j === 'country') {
                        return v[j].name_text
                    }
                    if (j === 'amount_payable') {
                        return v.amount * v.exchange_rate
                    }
                    if (j === "timestamp") {
                        return v[j]
                    } else {
