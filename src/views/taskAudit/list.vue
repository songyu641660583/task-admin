            ...
            <el-table border style="width: 100%;" stripe :data="data" v-loading="loading">
                <el-table-column align="center" type="expand" :label="$i18n.t('USER_TASK_INFO')"  fixed="left" :width="50">
                    <template slot-scope="props">
                        <el-form label-position="left" inline class="demo-table-expand">
                            ...
                            <el-form-item :label="$i18n.t('TASK_CATEGORY')" v-if="props.row.category !== null">
                                <span>{{ props.row.category.name_text }}</span>
                            </el-form-item>
                            <!--
                            ...
                            <el-form-item :label="$i18n.t('TASK_URL')">
                                <el-link :href="props.row.url" target="_blank" type="primary" icon="el-icon-link">{{ $i18n.t('CLICK_TO_VIEW') }}</el-link>
                            </el-form-item>
                ...
                <el-table-column align="center" prop="status" :label="$i18n.t('STATUS')" :min-width="100">
                    <template slot-scope="scope">
                        ...
                        <el-tag v-else-if="scope.row.status === 1" >{{ $i18n.t('USER_MANUAL_RECHARGE_STATUS_1') }}</el-tag>
                ...
                <el-table-column align="center" prop="updated_at" :label="$i18n.t('USER_TASK_AUDIT_TIME')" :min-width="150" v-if="params.type === '2' || params.type === '1'"></el-table-column>
                <el-table-column align="center" prop="admin_id" :label="$i18n.t('ADMIN_ID')" :min-width="150" v-if="params.type === '1' || params.type === '2'"></el-table-column>
                <el-table-column align="center" prop="remark" :label="$i18n.t('REMARK')" :min-width="150" v-if="params.type === '1' || params.type === '2'"></el-table-column>
                <!-- 右侧按钮 -->
                <el-table-column align="center" :label="$i18n.t('ACTION')" fixed="right" :width="200" v-if="params.type === '0' || params.type === '1' || params.type === '2'">
                    <template slot-scope="scope">
                        <el-link v-auth="'audit'" icon="el-icon-s-check" @click="onAudit(scope.row)" v-if="params.type === '0'">{{ $i18n.t('AUDIT') }}</el-link>
...
<script>
    ...
    import FormModal from '@/components/taskAudit/FormModal'
    import { enableTasks, disableTasks, deleteTasks } from '@/api/task'
    import { getTaskAuditList } from '@/api/taskAduit'
    ...
    export default {
        ...
        methods: {
            ...
            fetch () {
                this.loading = true
                getTaskAuditList({ ...this.params, page: this.page.currentPage, perPage: this.page.perPage }).then((res) => {
