    ...
    <div>
        ...
        <div class="table-body">
            ...
            <div class="table-left-top">
                <el-button v-auth="'add'" type="primary" icon="el-icon-plus" @click="onAdd">{{ $i18n.t('CREATE_PICTURE') }}</el-button>
            ...
            <el-table border style="width: 100%;" stripe :data="data" v-loading="loading" row-key="id" @selection-change="handleSelectionChange">
                ...
                <el-table-column align="center" prop="locale" :label="$i18n.t('LANGUAGE')" :width="100">
                    <template slot-scope="scope">
                        <span v-if="scope.row.country !== null">{{ scope.row.country.lang }}</span>
                        <span v-else>-</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="image" :label="$i18n.t('PICTURE')" :min-width="150">
                    <template slot-scope="scope">
...
<script>
    ...
    import { getInvitationList, deleteInvitation } from '@/api/invitation'
    import { getCountryList } from '@/api/country'
    import configs from '@/config'
    import ElImageViewer from 'element-ui/packages/image/src/image-viewer'
    import FormModal from '@/components/invitation/FormModal'
    ...
    export default {
        ...
        computed: {
            formItems () {
                ...
                // 构建搜索表单
                return [
                    { title: this.$i18n.t('LANGUAGE'), type: 'select_country_lang', key: 'locale', maxlength: 100, options: this.countryList }
                ]
            }
        },
        methods: {
            ...
            fetch () {
                this.loading = true
                getInvitationList({ ...this.params, page: this.page.currentPage, perPage: this.page.perPage }).then((res) => {
